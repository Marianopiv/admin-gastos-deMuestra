...
const VehiclesPanel = ({customClasses}:Props) => {
  ...
  return (
    <div className={`mb-5 text-left ${customClasses?"w-screen pl-10 sm:pl-0 sm:w-auto":""}`}>
      <div className={`mt-10 text-center mx-auto sm:flex justify-center lg:justify-normal ${customClasses?"":"lg:mx-80"}  items-center `}>
        <ButtonTransparent
      ...
      <div className={`flex flex-col items-center justify-center gap-10 mt-12`} >
        <div className={`flex flex-col ${!customClasses?"w-screen":""} gap-4 border-2 p-3 bg-gray-400 rounded-md`}>
          <div className={`flex bold text-lg uppercase ${customClasses?"gap-6 sm:gap-20":" gap-10"} h-14 items-center sm:w-full sm:justify-between sm:border rounded-md px-3 py-2 bg-white font-bold`}>
            ...
            <div className={`md:flex w-1/4 sm:w-1/6 justify-left ${customClasses?"absolute top-24 right-10 sm:relative sm:top-0 sm:right-0":""}`}>
              <button
                onClick={handleToogleSearch}
                className={`py-1 md:flex sm:w-24  text-xs sm:py-4  sm:px-1 text-center hover:bg-blue-50 hover:text-white rounded-md ${customClasses?"":"hidden"}`}
          ...
          {fullVehicles.length > 0 ? (
            sortByAlphabeth(fullVehicles)?.map((item, index) => (
              <>
                <div
                  key={index}
                  className={`flex gap-8 ${customClasses?"text-left sm:text-center":""} text-center md:text-left md:text-md items-center  sm:border rounded-md px-2 py-2 bg-white lg:py-auto`}
                >
